...
class SafeMoveServiceServer
{
public:
  SafeMoveServiceServer()
  {
    ...
    mSafeMoveService = mNode.advertiseService("/arm_mover/safe_move", &SafeMoveServiceServer::handle_safe_move_request, this);
...
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2)
{
    ...
    mNode.getParam(node_name + "/min_joint_1_angle", min_j1);
    mNode.getParam(node_name + "/max_joint_1_angle", max_j1);
    mNode.getParam(node_name + "/min_joint_2_angle", min_j2);
    mNode.getParam(node_name + "/max_joint_2_angle", max_j2);
  ...
  ros::ServiceServer mSafeMoveService;

};//End of class SafeMoveServiceServer
...
int main(int argc, char** argv)
{
    ...
    // Create an object of class SafeMoveServiceServer that will take care of everything
    SafeMoveServiceServer smServiceServerObj;
